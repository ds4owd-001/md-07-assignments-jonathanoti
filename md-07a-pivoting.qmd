...
2.  What's the `tidyverse` Package? Describe in maximum two sentences below.
    ...
    **The tidyverse is a compilation of several packages that are designed to work together for efficient data manipulation, exploration, and visualization.**
...
1.  Starting with the `waste_data_long_mean` object, execute the code and observe the output in the Console. Would you publish this table in this format in a report? **No. It is hard to read.**
...
2.  Execute the code and observe the output in the Console. Would you publish this table in a report in this format? **Yes, but not in the way it is displayed (table appearance). The structure is much better to read and understand than the long format.**
